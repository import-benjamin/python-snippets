...
for img_name in img_set:
    ...
    print("loaded image : {}, {}x{}px".format(img_name, img_rgb[img_name].shape[0], img_rgb[img_name].shape[1]))
template_rgb = cv2.imread('Data-TP/waves.jpg', 0)

template_hist = cv2.calcHist(template_rgb, [0], None, [256], [0, 256]) # get template histogram

for img_name in img_set:
    img_hist["{}".format(img_name)] = cv2.calcHist(img_rgb[img_name], [0], None, [256], [0, 256])
    result[img_name] = cv2.compareHist(template_hist, img_hist[img_name], cv2.HISTCMP_INTERSECT)
...
print("Closest image: (using intersect) -> {} with {} similarity".format(img, max(result.values())))
