...
candidates = [x for x in hist_cmp if x[0] >= correlation_minimum] #max(hist_cmp, key=lambda x: x[0])
print(candidates)

for scope in candidates:
    cv2.rectangle(img_rgb, scope[1], scope[2], (random.randrange(0, 255), random.randrange(0, 255), random.randrange(0, 255)), 2)
