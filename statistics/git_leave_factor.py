        ...
        Counter(
            [
                ...
                for c in subprocess.run(
                    f"git blame --line-porcelain HEAD {file} | grep \"^{condition} \" | sort | cut -d' ' -f2-",
                    shell=True,
...
def main():
    ...
    print(
        *[
            f"{key:<35} : {round(float(authorship_percent[key])*100, 2)}%"
            for key in authorship_percent
        ],
        sep="\n",
    )
    print(
        f"{' total : '+str(round(float(sum(authorship_percent.values())*100), 2))+'% ':=^80}"
    )
